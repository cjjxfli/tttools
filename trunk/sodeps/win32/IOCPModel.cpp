...
{
	memset(m_func_list_, 0, sizeof(cb_func)* (CB_TIMEOUT + 1));
...
{    
	...
	//是否要改为1字节否则，请看：http://blog.csdn.net/thebluebluesky/article/details/6081907
	DWORD                dwBytesTransfered = 0/*0*/; 
	...
	while (WAIT_OBJECT_0 != WaitForSingleObject(pIOCPModel->m_hShutdownEvent, 0))
	{
		...
		{  	
			...
			{  
				...
				if (pIOCPModel->m_func_list_[CB_DISCONNECT] != NULL)
				{
					PER_SOCKET_CONTEXT * pSocket = pIOCPModel->_GetSocketContext(pIoContext->m_sockAccept);
					if (pSocket && pIOCPModel->m_func_list_[CB_DISCONNECT](pIoContext->m_sockAccept, &(pSocket->m_ClientAddr), pIOCPModel->m_func_args_[CB_DISCONNECT]) != 0)
					{
						//print error
					}
				}
...
bool CIOCPModel::_InitializeListenSocket()
{
	...
	m_pListenContext->m_Socket = WSASocket(AF_INET, SOCK_STREAM, IPPROTO_IP, NULL, 0, WSA_FLAG_OVERLAPPED);
...
bool CIOCPModel::_PostAccept( PER_IO_CONTEXT* pAcceptIoContext )
{
	...
	pAcceptIoContext->m_sockAccept = WSASocket(AF_INET, SOCK_STREAM, IPPROTO_IP/*IPPROTO_TCP*/, NULL, 0, WSA_FLAG_OVERLAPPED);
	...
	if(FALSE == m_lpfnAcceptEx( 
		...
		p_wbuf->buf, 
		p_wbuf->len - ((sizeof(SOCKADDR_IN)+16) * 2),//0,//接受连接即返回/*p_wbuf->len - ((sizeof(SOCKADDR_IN)+16)*2*/),   
...
bool CIOCPModel::_DoAccpet( PER_SOCKET_CONTEXT* pSocketContext, PER_IO_CONTEXT* pIoContext )
{
	...
	//是否也要设置一下pSocketContext？
// 	pSocketContext->m_Socket = pIoContext->m_sockAccept;
// 	memcpy(&(pSocketContext->m_ClientAddr), ClientAddr, sizeof(SOCKADDR_IN));
	...
	//6. 如果有注册接受连接的回调函数，执行之
	if (m_func_list_[CB_CONNECTED] != NULL)
	{
		if (m_func_list_[CB_CONNECTED](pNewSocketContext->m_Socket, &(pNewSocketContext->m_ClientAddr), m_func_args_[CB_CONNECTED]) != 0)
		{	
			//print error
		}
	}
...
bool CIOCPModel::_PostRecv( PER_IO_CONTEXT* pIoContext )
{
	...
	buffer_blob.data = p_wbuf->buf;
	buffer_blob.len = p_wbuf->len;
	buffer_blob.owner = m_func_args_[CB_RECVDATA];
...
// 	pIoContext->m_OpType = RECV_POSTED;
	...
	if (NULL != m_func_list_[CB_RECVDATA])
	{
		m_func_list_[CB_RECVDATA](pIoContext->m_sockAccept, &buffer_blob, m_func_args_[CB_RECVDATA]);
	}
	return true;
}
...
bool CIOCPModel::_PostSend(PER_IO_CONTEXT* pIoContext)
{
	...
	if ((SOCKET_ERROR == nBytesRecv) && (WSA_IO_PENDING != err_code))
	{
		CString err_msg;
		err_msg.Format("%s %d", "投递第一个WSASend失败！", err_code);
		this->_ShowMessage(err_msg);
		return false;
	}
	if (NULL != m_func_list_[CB_SENDDATA])
	{
		blob_type buffer_blob;
		buffer_blob.data = p_wbuf->buf;
		buffer_blob.len = p_wbuf->len;
		buffer_blob.owner = m_func_args_[CB_SENDDATA];
		m_func_list_[CB_SENDDATA](pIoContext->m_sockAccept, &buffer_blob, m_func_args_[CB_SENDDATA]);
	}
	return true;
}
...
bool CIOCPModel::_DoRecv( PER_SOCKET_CONTEXT* pSocketContext, PER_IO_CONTEXT* pIoContext )
{
	...
	//重复接收数据时，这里会异常，pSocketContext中无客户端信息
#if 0
...
#endif
	int sock_fd = pIoContext->m_sockAccept;
	{
		PER_SOCKET_CONTEXT * socket_context = _GetSocketContext(sock_fd);
		if (NULL != socket_context)
		{
			SOCKADDR_IN* ClientAddr = &socket_context->m_ClientAddr;
			this->_ShowMessage(_T("收到  %s:%d 信息：%s"), inet_ntoa(ClientAddr->sin_addr), ntohs(ClientAddr->sin_port), pIoContext->m_wsaBuf.buf);
		}
	}
...
int CIOCPModel::DoSend(unsigned int sock_fd, const char * buffer, unsigned int len)
{
	_PER_IO_CONTEXT * io_context = _GetIOContext(sock_fd);
	if (NULL == io_context)
	{
		return -1;
	}
	PER_SOCKET_CONTEXT* pSocketContext = _GetSocketContext(sock_fd);
	...
	int ret = io_context->CopyDataToBuffer(buffer, len);
	if (0 == ret)
	{
		_DoSend(pSocketContext, io_context);
	}
	return ret;
}
...
void CIOCPModel::_RemoveContext( PER_SOCKET_CONTEXT *pSocketContext )
{
	...
	std::vector<PER_SOCKET_CONTEXT*>::iterator it = m_arrayClientContext.begin();
...
_PER_IO_CONTEXT * CIOCPModel::_GetIOContext(unsigned int sock_fd)
{
	std::vector<PER_SOCKET_CONTEXT*>::iterator it = m_arrayClientContext.begin();
	for (; it != m_arrayClientContext.end(); it++)
	{
		_PER_IO_CONTEXT * io_context = (*it)->GetIOContext(sock_fd);
		if (NULL != io_context)
		{
			return io_context;
		}
...
PER_SOCKET_CONTEXT * CIOCPModel::_GetSocketContext(unsigned int sock_fd)
{
	std::vector<PER_SOCKET_CONTEXT*>::iterator it = m_arrayClientContext.begin();
	for (; it != m_arrayClientContext.end(); it++)
	{
...
// 	}	
	if (m_cb_error != NULL)
	{
		m_cb_error(strMessage.GetBuffer(),strMessage.GetLength(),m_cb_error_args);
	}
	//m_msg_info = strMessage;
}
